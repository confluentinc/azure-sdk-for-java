            ...
            .onErrorResume(throwable -> {
                ...
                if (shouldRetry(throwable, sinkOperation.getRetryCount(), this.writeConfig.getMaxRetryCount())) {
                    ...
                    if (this.writeConfig.getToleranceOnErrorLevel() == ToleranceOnErrorLevel.ALL) {
                        // A CosmosException.toString() renders resourceAddress (.../docs/{id}) with a
                        // record-derived id (customer data), so it must never be logged raw. A
                        // non-Cosmos throwable has no resourceAddress, so log it in full (with stack);
                        // only sanitize when a CosmosException is present.
